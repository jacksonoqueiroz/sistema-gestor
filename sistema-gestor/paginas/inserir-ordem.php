...
?>
<title>Inserir Ordem Fabricação</title>
</head>
<?php
...
<div class="container py-4 cadastro">
	<header class="pb-3 mb-4 border-bottom">
		...
		<div class="row">
			...
			<h2>Inserir Fabricação</h2>
			</div>
			<div class="row">
				<div class="col-md-3 list-icon">
					<a href="<?php echo URL ?>listar-in-ordem"><i class="fa fa-list"></i></a>
				</div>
				<div class="col-md-3 list-icon">
					<a href="<?php echo URL ?>cadastrar-ordem_producao"><i class="fa fa-clipboard"></i></a>
			</div>
			</div>
		...
		if (!empty($dados['cad_reg'])) {
			...
			if(!$empty_input) {
				...
				$query_cad = "INSERT INTO inserir_ordem (id_ordem, id_peca, qtd, usuario) VALUES (:id_ordem, :id_peca, :qtd, :usuario)";
        ...
        $cad_reg->bindParam(':id_peca', $dados['peca'], PDO::PARAM_STR);
				$cad_reg->bindParam(':qtd', $dados['qtd'], PDO::PARAM_STR);
	...
	<form name="cad_op" class="form-group" method="POST" action="">
      ...
      <label for="operacoes">Ordem:</label>
     ...
     <select name="ordem" id="ordem" class="form-control" required>
      <?php
      $query = $conn->query("SELECT ordem.id,
                          ordem.codigo,                          
                          modelo.nome AS modelo 
                  FROM ordem_producao AS ordem
                  INNER JOIN produtos AS modelo ON modelo.id=ordem.id_modelo ORDER BY codigo ASC");
     ...
     <select name="peca" id="peca" class="form-control" required>
	  ...
	  <div class="col-2">
	  	<label for="tempo" class="form-label">Quantidade</label>
		    <input type="number" name="qtd" autocomplete="off" class="form-control" id="qtd">
	  </div>
