...
?>
<title>Ordem de Fabricação OF'S</title>
</head>
<?php
...
<div class="container py-4">
	<header class="pb-3 mb-4">		
		...
		<div class="row">
			...
			<h2>Ordem de fabricação OF'S</h2>
			</div>
			<div class="col-md-3 list-icon">
				<a href="<?php echo URL ?>inserir-ordem"><i class="fa fa-clipboard"></i></a>
			</div>
    ...
    <?php 
    	if(isset ($_SESSION['msg'])){
    		...
    		unset($_SESSION['msg']);
					...
					$query_ordem = "SELECT ordem.id AS id_ordem,
                          ordem.codigo,                       
                          modelo.nome AS modelo 
                  FROM ordem_producao AS ordem
                  INNER JOIN produtos AS modelo ON modelo.id=ordem.id_modelo";
					$result_ordem = $conn->prepare($query_ordem);
					$result_ordem->execute();
					$row_ordem = $result_ordem->fetch(PDO::FETCH_ASSOC);
					extract($row_ordem);
    ...
    <table class="table table-striped">
            <thead>
              <tr>
              	<th colspan="4">Ordem: <?php echo $codigo ?> - Modelo: <?php echo $modelo ?></th>
              </tr>
            </thead>
              		<tr>
              			<th scope="col">Peça</th>
                		<th scope="col">Qtd Produção</th>
    ...
    $query_reg = "SELECT inserir_ordem.id_ordem,
                            ...
                            peca.nome AS peca,
                            inserir_ordem.id_peca,
                            inserir_ordem.qtd,
                            inserir_ordem.status
                FROM inserir_ordem AS inserir_ordem
                INNER JOIN peca AS peca ON peca.id=inserir_ordem.id_peca
                INNER JOIN ordem_producao AS ordem ON ordem.id=inserir_ordem.id_ordem LIMIT $inicio, $limite_resultado";
              ...
              <tr>
                ...
                <td class="row">
                	<div class="col-2 action-icon">
                		<a href="<?php echo URL . 'fabricacao?id_ordem=' . $id_ordem . '&id_peca=' . $id_peca?>"><i class="fa fa-eye"></i></a>
                		</div>
                	<div class="col-2 action-icon">
                		<a href="<?php echo URL . 'editar-item?id_material=' . $id_ordem?>"><i class="fa fa-pen"></i></a>
                		</div>
                	<!-- <div class="col-2 action-icon">
                			<a href="<?php echo URL . 'excluir-item?id_material=' . $id . ', nome=' . $nome ?>"><i class="fa fa-trash" onclick=""></i></a>
                		</div> -->
                		<div class="col-2 action-icon">
                			<a onclick="apagarRegistro(<?php echo $id ?>)" href="<?php echo URL . 'excluir-item?id_material=' . $id_item ?>"><i class="fa fa-trash"></i></a>
                		</div>
                </td>
                </tr>
	...
	?>
		...
		//Contar a quantidade de registros no Banco de Dados
			$query_qnt_resgistros = "SELECT count(id) AS num_result FROM inserir_ordem";
  ...
  <ul class="pagination">
    <li class="page-item">
      ...
      <?php echo "<a class='page-link' href='peca-ordem?page=1'>Primeira</a> "; ?>
    </li>
    ...
    <li class="page-item">
    	...
    	<?php
				if ($pagina_anterior >= 1) {
					echo "<a class='page-link' href='peca-ordem?page=$pagina_anterior'>$pagina_anterior</a> ";
				}
    ...
    <?php 
		...
		<li class="page-item">
    		<?php
							if($proxima_pagina <= $qnt_pagina){
								echo "<a class='page-link' href='peca-ordem?page=$proxima_pagina'> $proxima_pagina </a> ";
							}
    ...
    <li class="page-item">
      <?php echo "<a class='page-link' href='peca-ordem?page=$qnt_pagina'>Última</a> "; ?>
    </li>
  </ul>
</nav>
